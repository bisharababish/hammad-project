...
function BookingPage() {
  ...
  const { data: slotCounts } = useQuery({
    ...
    queryFn: async () => {
      ...
      return Object.fromEntries(
        (data ?? []).map((r) => [r.booking_time, Number(r.cnt)]),
      );
    },
  });
  ...
  async function submit() {
    ...
    if (!selDate || !selTime) {
      toast.warning(
        lang === "ar" ? "اختر التاريخ والوقت" : "Pick date and time",
      );
      return;
    }
    setSubmitting(true);
    ...
    >
      <div className="mb-10">
        ...
        <h2
          ...
          style={{ fontFamily: "Cairo", fontSize: "clamp(26px,3.2vw,42px)" }}
        >
      ...
      <div className="grid lg:grid-cols-[1.1fr_.9fr] gap-6 items-start">
        ...
        <div className="card-surface p-7">
          <div
            style={{ fontSize: 19, fontWeight: 700, color: "var(--gold-dark)" }}
          >
            {tr("booking.details")}
          </div>
                ...
                <button
                  ...
                  style={{
                    border: sel
                      ...
                      : "1.5px solid rgba(201,168,76,.12)",
                    background: sel ? "var(--surface-alt)" : "var(--surface)",
                  }}
                >
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600 }}>
                      {s.emoji} {lang === "ar" ? s.name_ar : s.name_en}
                    </div>
                    <div
                      style={{
                        fontFamily: "Poppins",
          ...
          <div className="grid sm:grid-cols-2 gap-3 mt-5">
            <input
              ...
              placeholder={tr("booking.fullName")}
              value={name}
              onChange={(e) => setName(e.target.value)}
            />
            <input
              className="form-input"
              placeholder={tr("booking.phone")}
              value={phone}
              onChange={(e) => setPhone(e.target.value)}
            />
            <input
              className="form-input"
              placeholder={tr("booking.carType")}
              value={car}
              onChange={(e) => setCar(e.target.value)}
            />
            <input
              className="form-input"
              placeholder={tr("booking.color")}
              value={color}
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              >
                <ChevronRight className="flip-rtl" size={18} />
              </button>
              <div
                style={{
                  fontFamily: "Poppins",
              ...
              <button
                ...
                style={{ color: "var(--gold-dark)" }}
              >
                <ChevronLeft className="flip-rtl" size={18} />
              </button>
            </div>
        ...
        <div className="card-surface p-7 lg:sticky lg:top-[82px]">
          <div
            style={{ fontSize: 17, fontWeight: 700, color: "var(--gold-dark)" }}
          >
            {tr("booking.yours")}
          </div>
          <div className="mt-4 flex flex-col gap-2.5 max-h-[600px] overflow-y-auto">
            ...
            {user && (!myBookings || myBookings.length === 0) && (
              <div
                ...
                style={{ color: "var(--text-muted)" }}
              >
                📋<div className="mt-2 text-sm">{tr("booking.empty")}</div>
              </div>
            )}
            {myBookings?.map((b) => (
              <div
              ...
              >
                <div
                  className="absolute top-0 inset-inline-end-0 w-[3px] h-full"
                  style={{
                    background:
                      "linear-gradient(to bottom,var(--gold),var(--gold-dark))",
                    insetInlineEnd: 0,
                  }}
                />
