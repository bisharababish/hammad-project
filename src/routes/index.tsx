  ...
  head: () => ({
    meta: [
      { title: "مغسلة البرنس | Al-Prince Car Wash" },
      {
        name: "description",
        content:
          "Premium car wash and detailing in your city. Book in under a minute.",
      },
    ],
  }),
  component: Home,
...
function Home() {
  ...
  const { data: stats } = useQuery({
    queryKey: ["home-stats"],
    queryFn: async () => {
      const [clientsRes, reviewsRes, settingsRes, servicesRes] =
        await Promise.all([
          supabase
            .from("profiles")
  ...
  const statItems = [
    stats?.clientCount
      ? { n: `${stats.clientCount}+`, k: "hero.clients" }
      : null,
    ...
    <>
      <section className="relative overflow-hidden px-[8%] py-16 md:py-24 min-h-[calc(100vh-68px)] flex items-center">
        ...
        <div className="relative z-10 max-w-[640px]">
          ...
          <p
            className="my-8 leading-[1.9] animate-fade-up"
            style={{ fontSize: 15, color: "var(--text-muted)", maxWidth: 500 }}
          >
            {tr("hero.desc")}
          </p>
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-2">
                {services.slice(0, 3).map((s) => (
                  <div
                    key={s.id}
                    className="text-center p-2.5 rounded-[10px]"
                    style={{
                      background: "var(--surface-alt)",
                      border: "1px solid var(--border-c)",
                    }}
      ...
      >
        <div className="max-w-[560px] mx-auto">
          ...
          <h2
            className="mt-3 mb-4 font-black"
